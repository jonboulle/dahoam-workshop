...
Follow the [Kubernetes + CoreOS + AWS docs](https://coreos.com/kubernetes/docs/latest/kubernetes-on-aws.html). At the `kube-aws init` step use mycluster.example.com as the `external-dns-name` unless you have a registered domain you wish to use for this cluster and "mycluster" as the `cluster-name`. The rest of the tutorial will depend on these two values.
